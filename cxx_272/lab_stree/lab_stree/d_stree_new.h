...
#include "d_stiter.h"
        ...
        int countInSubtree(const T& item);

		void countNodes(stnode<T> *t, int& count);
...
template <typename T>
int stree<T>::countInSubtree(const T& item)
{
    int count = 0;
	stnode<T> *found;
	...
	if (found != NULL)
	{
		countNodes(found, count);
		//countNodes includes root node
		count--;
	}
...
template <typename T>
void stree<T>::countNodes(stnode<T> *t, int& count)
{
	if (t != NULL)
	{
...
#endif  // BINARY_SEARCH_TREE_CLASS#include <iostream>
